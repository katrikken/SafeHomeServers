...
AS
  CURSOR C IS
    SELECT TO_CHAR(ACTION_DATE, 'DD-MM-YYYY HH24:MI:SS.FF') AS ACTION_DATE, ACTION_INFO, ACTION_LEVEL
        FROM RPI_ACTIONS
          WHERE RPI_ID = P_RPI_ID AND ACTION_DATE <= P_ACTION_DATE AND ROWNUM <= P_COUNT
...
CREATE FUNCTION GET_LATEST_DATE_ON_ACTIONS(
    P_RPI_ID RPI_DEVICES.RPI_ID%TYPE -- RPI ID
  ) RETURN RPI_ACTIONS.ACTION_DATE%TYPE  -- TIMESTAMP
AS
  RETVAL RPI_ACTIONS.ACTION_DATE%TYPE;
BEGIN
  SELECT ACTION_DATE INTO RETVAL
	FROM RPI_ACTIONS
	 WHERE RPI_ID = P_RPI_ID AND ROWNUM = 1
	   ORDER BY ACTION_DATE DESC;
...
CREATE TABLE RPI_PHOTOS(
  ...
  PHOTO_NAME CHARACTER VARYING(20) NOT NULL,
...
AS
  CURSOR C IS
    SELECT TO_CHAR(PHOTO_DATE, 'DD-MM-YYYY HH24:MI:SS.FF') AS PHOTO_DATE, PHOTO_NAME
        FROM RPI_PHOTOS
          WHERE RPI_ID = P_RPI_ID AND PHOTO_DATE <= P_PHOTO_DATE AND ROWNUM <= P_COUNT
...
BEGIN
  ...
  FOR R IN C LOOP
    RETVAL := RETVAL || DELIM 
	  || TIMEV || QUOTM || R.PHOTO_DATE || QUOTM;
	DELIM := ',';
	RETVAL := RETVAL || DELIM
...
CREATE FUNCTION GET_LATEST_DATE_ON_PHOTOS(
    P_RPI_ID RPI_DEVICES.RPI_ID%TYPE -- RPI ID
