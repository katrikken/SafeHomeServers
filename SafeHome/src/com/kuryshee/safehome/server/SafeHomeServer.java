 ...
 */
@WebServlet(loadOnStartup = 1, urlPatterns = {"/SafeHomeServer", "/SafeHomeServer/rpi/*"})
@MultipartConfig
public class SafeHomeServer extends HttpServlet {
    ...
    /**
     * The answer for the request, which checks the availability of the server. 
     */
    public static final String PING_ANSWER ="ping";
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*if(path.startsWith(servletPathApp, 0)){
			...
			if(command.equals(SafeHomeServer.REQ_HISTORY) || command.equals(SafeHomeServer.REQ_LIST)){
				AppRequestProcessor appprocessor = new AppRequestProcessor();
				appprocessor.sendText(response, command, query, getServletContext());
			}
			else if(command.equals(SafeHomeServer.REQ_DOWNLOAD)){
				AppRequestProcessor appprocessor = new AppRequestProcessor();
				appprocessor.sendPhoto(response, query, getServletContext());
			}		
			else{		
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*else if(path.startsWith(servletPathApp, 0)) {
			log.info("-- Application POST");
			AppRequestProcessor processor = new AppRequestProcessor();
